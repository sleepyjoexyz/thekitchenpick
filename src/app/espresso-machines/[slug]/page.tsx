    ...
    <article className="bg-white">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 border-t border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {getAllEspressoMachineArticleSlugs()
            .filter((s) => s !== slug)
            .slice(0, 4)
            .map((relatedSlug) => {
              const related = getEspressoMachineArticle(relatedSlug);
              if (!related) return null;
              return (
                <Link
                  key={relatedSlug}
                  href={`/espresso-machines/${relatedSlug}`}
      ...
      <CrossCategoryLinks currentCategory="espresso-machines" />
